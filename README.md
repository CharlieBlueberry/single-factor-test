...
> 从笔试代码蒸馏出的可复用单因子检验操作系统。覆盖从因子构造到QP建仓的完整链路。
...
数据区间：2010—2022，A股全市场，日频截面。

### 分层净值（Raw Z-Score & Neutralized）

![四合一回测图](images/factor_analysis_4in1.png)

- **Layer 0（因子最小，反转多头端）** 净值持续跑赢，2022年累计约 2.2×
- **Layer 4（因子最大，动量端）** 净值趋近于 0，反转效应显著
- Raw 与 Neutralized 结果高度一致 → 因子 alpha 来自本身，非规模/行业暴露

### 行业中性化分层净值

![行业中性分层](images/industry_neutral_layer_backtest.png)

- 行业内分层单调性更干净，组合1（低因子值）持续超额
- 行业中性化后极端层分离度更明显，降低了行业轮动噪音

### IC 结论

Cum IC 持续向下（负 IC），与分层回测一致 —— 因子方向为**反转**（因子值小的股票收益更高）。
IC 均值约 -0.025，IR < -0.5，全周期统计显著。
